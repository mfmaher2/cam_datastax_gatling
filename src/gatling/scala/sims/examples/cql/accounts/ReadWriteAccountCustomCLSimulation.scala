...
class ReadWriteAccountCustomCLSimulation  extends BaseSimulation {
  ...
  val writeFeed = acctFeed.writeFakeAccount
  val readFeed = acctFeed.readAccoutnNum

  val writeAccountScenario = scenario("AccountWrite")
    .feed(writeFeed)
    .exec(accountActions.writeAcountOpcoWithCustomConsistencyLevel("FX", ConsistencyLevel.LOCAL_QUORUM))
    .exec(accountActions.writeAcountOpcoWithCustomConsistencyLevel("FX", ConsistencyLevel.EACH_QUORUM))
